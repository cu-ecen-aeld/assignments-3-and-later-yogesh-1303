/*man pages for socket related functions and beej guide website are referred to while making this code */ 
...
int main()
{
	...
	if((sc = getaddrinfo(NULL, PORT, &hints, &res)) != 0)
	{
		perror("\ngetaddrinfo");
		return -1;
	}
	...
	// while(*(buf+count) != '\0')
	// {
	//receive until the \0 character occurs
	// for(;;)
	// {
	// 	int i;
	// 	//receive 10 bytes at a time and store them in a dynamically allocated buffer
	// 	if(recv(new_fd, shortest_buf, strlen(shortest_buf), 0) == -1)
	// 	{
	// 		perror("\nrecv");
	// 		return -1;
	// 	}
		
	// 	for(i=0; i<strlen(shortest_buf); i++)
	// 	{
	// 		if(shortest_buf[i] != '\n')
	// 			*(buf + count + i) = shortest_buf[i];
	// 		else
	// 			break;
	// 	}
	// 	if(shortest_buf[i] == '\n')
	// 	{
	// 		wr = write(fd, buf, (count + i + 1));
	// 		if(wr == -1)
	// 		{
	// 			perror("\nwrite");
	// 			return -1;
	...
	// while(*(buf+count) != '\0')
	// {
	if((wr = write(fd, &shortest_buf, strlen(shortest_buf))) == -1)
	{
		perror("\nwrite");
		exit(1);
	}
